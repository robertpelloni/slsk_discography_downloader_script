...
- **Protocol Automation**: Enhanced branch reconciliation to handle multi-remote upstream/origin sync.
...
- **Thread Safety**: Added `_run_in_thread` with asyncio.Semaphore(3) and cancel events to prevent thread pool exhaustion from orphan `to_thread` workers.
- **Socket Timeout**: Applied `socket.setdefaulttimeout(15)` globally to prevent MusicBrainz API calls from hanging indefinitely.
- **Cancel Events**: `get_discography` and `get_related_artists` now accept cancel_event callbacks for early bail-out on timeout.
- **Filler Isolation**: Moved autonomous filler to a detached subprocess (`filler_worker.py`) so it can't crash the web server.
- **Watchdog Fixes**: Server starts now use system Python to avoid venv stub-process duplication; `kill_process` added wmic fallback.
- **API Cooldown**: Added 5-minute cooldown to `/api/autonomous_fill` with proper HTTP 429 response.
- **Frontend UX**: Fill button now shows status messages in log instead of hiding silently.
